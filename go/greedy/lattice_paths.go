...
func KDistinct(k int, path1, path2 []bool) bool {
	...
	if k == 0 {
		return false
	} // if k is zero, no paths will be k distinct
	...
	for i := 0; i < size; i++ { // iterate over the paths
		...
		if path1[i] {
			path1north++
		} // if path 1 steps north, increment its north steps
		if path2[i] {
			path2north++
		} // if path 2 steps north, increment its north steps
	}
...
func KDistinctInSet(k int, path []bool, path_set [][]bool) bool {
	...
	for _, x := range path_set { // we loop over the sets in the path set
		if !(KDistinct(k, path, x)) {
			return false
		} // if the path we are comparing is not k distinct with the current path from the set, return false
	}
...
func CombinationsProcess(m, n, size int, ch chan [][]bool) {
	...
	if size > num {
		return
	} // we can't form a set with more paths than we have
