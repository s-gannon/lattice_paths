...
find_all_distinct_sets(m: int, n: int)
    ...
    for all k
greedy_max_comparison(m: int, n: int)
    Get a table comparing the greedy algorithm to the maximum for all k.
generate_data(starting_dimension: int)
...
def greedy_max_comparison(m,n,k):
    """
    Get a dictionary comparing the greedy algorithm to the maximum for some k.
    ...
    -------
    dict
        A dictionary containing the data generated by the greedy algorithm and
    ...
    if up_bound == result['greedy_cardinality']:
        ...
        result['max_sets'] = find_distinct_sets(m,n,k,result['max_cardinality'])
    else:
        size = up_bound
        ...
        while distinct_sets is None:
            size -= 1
            distinct_sets = find_distinct_sets(m,n,k,size)
        result['max_cardinality'] = size
        result['max_sets'] = distinct_sets
        result['greedy_is_max']=size==result['greedy_cardinality']
    return result


def greedy_max_comparison_table(m,n):
    """
    Get a table comparing the greedy algorithm to the maximum for all k.
    ...
    -------
    pandas.DataFrame
        A dataframe containing the data generated by the greedy algorithm and
        the brute force approach.
    """
    results = DataFrame(columns = ['m','n','k','greedy_cardinality',
                                   ...
                                   'greedy_is_max'])
    for k in range(m+n+1):
        results = greedy_max_comparison(m,n,k)
    return results
